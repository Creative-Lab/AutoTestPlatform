...
public class StepSimulator {
	...
	private String testSimulatorKeyword = "";
	...
	public StepSimulator(String testDriverKeyword,HashMap<String, String> objDefenition,ResourceManager rmanager){
		this.rm = rmanager;
		
		this.testSimulatorKeyword = testDriverKeyword; 
		
		this.objDefInfo = null;
		
		this.objDefInfo = objDefenition;
	...
	private void enableTestDriver(){
		TestDriverFactory tdFactory = new TestDriverFactory(testSimulatorKeyword,rm);
		
		testSimulator = null;
		
		testSimulator = tdFactory.getTestDriver();
	}
	
	/**
	 * @author Nayan
	 * @param testData : String passed by user as a test data.</br>
	 * <b> Description: </b> </br> Parse and filter test data according to, whether test data is specified in form of "Property:Value"  or just the plain test data.
	 * 
	 */
	...
	public void simulateTestStep(String stepAction,String testData,String testObject, String strategyModifier,boolean isReusableTestKeyword) throws Exception{
		...
		testSimulator.setObjectDefenition(this.objDefInfo); // Setting up the object definition.
